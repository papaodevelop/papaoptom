...
class FileManagerPlugin
{
    ...
    public function aroundCreateBundleJsPool(\Magento\RequireJs\Model\FileManager $subject, callable $proceed)
    {
        ...
        $bundleFile = $context->getPath() . '/' . \Amasty\Checkout\Model\Optimization\Bundle::BUNDLE_JS_DIR . '/'
        ...
        if (!$staticDir->isExist($bundleFile) || !$staticDir->isFile($bundleFile)) {
            return [];
        }

        $this->bundleService->setBundleLoaded();

        $relPath = $staticDir->getRelativePath($bundleFile);
