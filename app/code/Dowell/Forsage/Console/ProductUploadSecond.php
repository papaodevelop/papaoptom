...
class ProductUploadSecond extends Command
{
...
//        \Magento\Framework\Event\ManagerInterface $eventManager,
    ...
    function configure()
    {
        ...
        $this->setName('product:uploadsecond')
...
//                        die();
        ...
        /*foreach ($suppliers->suppliers as $supplier) {
            if (isset($supplier->id)) {
                if ($supplier->id>139) {//42 11 44 95 93 //124
...
//                        die();
        ...
        /*foreach ($suppliers->suppliers as $supplier) {
            if (isset($supplier->id)) {
                if (($supplier->id>87) && ($supplier->id<97)) {//42 11 44 95 93
                    $products = $connection->GetProductsBySupplier($supplier->id);
                    if (isset($products->product_ids)) {
                        foreach ($products->product_ids as $product_id) {
                            $product_request = $connection->GetProduct($product_id);
...
//                        die();
        ...
        foreach ($suppliers->suppliers as $supplier) {
            if (isset($supplier->id)) {
                if ($supplier->id>1060) {//42 11 44 95 93
                    $products = $connection->GetProductsBySupplier($supplier->id);
                    if (isset($products->product_ids)) {
                        foreach ($products->product_ids as $product_id) {
                            $product_request = $connection->GetProduct($product_id);
